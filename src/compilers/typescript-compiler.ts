...
// Analyze TypeScript code for input() calls
const analyzeCode = (sourceCode: string): InputAnalysis => {
  ...
  lines.forEach((line, lineIndex) => {
    // Check for console.log prompts (e.g., console.log("Enter your name:"))
    const logMatch = line.match(/console\.log\s*\(\s*["']([^"']+)["']\s*\)/);
    if (logMatch) {
      lastPrompt = logMatch[1];
    }

    // Check for readline() calls in Node.js context
    // Matches: readline(), prompt(), etc.
    const inputMatch = line.match(/readline\s*\(\s*\)/) || line.match(/prompt\s*\(\s*["']?([^"')]*?)["']?\s*\)/);
    if (inputMatch) {
...
export const typescriptCompiler: Compiler = {
  type: 'typescript',
  useTerminal: true,
  ...
  compile: async (files: FileData[], entryFile: string): Promise<CompilerResult> => {
    ...
    return {
      success: true,
      output: 'TypeScript preview ready'
    };
  },
  ...
  compileWithStdin: async (files: FileData[], entryFile: string, stdin: string = ''): Promise<CompilerResult> => {
    const mainFile = files.find(f => f.name === entryFile) || files.find(f => f.name.endsWith('.ts'));

    if (!mainFile) {
      return {
        success: false,
        output: '',
        error: 'No TypeScript file found to compile.'
      };
    }
    ...
    try {
      ...
      const createResponse = await fetch(`${JUDGE0_API}/submissions`, {
        ...
        body: JSON.stringify({
          ...
          language_id: LANGUAGE_IDS.typescript,
          stdin: stdin,
          wait: false,
